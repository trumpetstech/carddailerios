...
class FavouritesDatabase {
  ...
  static final _databaseName = "CardDailer.db";
  ...
  Future<List<Favourite>> queryAllRows() async {
    Database db = await instance.database;
    var allRows = await db.query(table);
      
    List<Favourite> favourites = allRows.isNotEmpty ? allRows.map((c) => Favourite.fromMap(c)).toList() : [];
    print('query all rows:' + favourites.toString());
    return favourites;
